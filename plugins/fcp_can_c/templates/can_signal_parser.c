...
uint32_t decode_signal_unsigned_as_uint32_t(CanFrame msg, uint64_t start,
                                            uint64_t length, double scale,
                                            double offset) {
    uint64_t bitfield = get_bitfield(can_word(msg), start, length);
    return apply_linear_uint64_t(bitfield, scale, offset);
...
uint16_t decode_signal_unsigned_as_uint16_t(CanFrame msg, uint64_t start,
                                            uint64_t length, double scale,
                                            double offset) {
    uint64_t bitfield = get_bitfield(can_word(msg), start, length);
    return apply_linear_uint64_t(bitfield, scale, offset);
...
uint8_t decode_signal_unsigned_as_uint8_t(CanFrame msg, uint64_t start,
                                          uint64_t length, double scale,
                                          double offset) {
    uint64_t bitfield = get_bitfield(can_word(msg), start, length);
    return apply_linear_uint64_t(bitfield, scale, offset);
...
double decode_signal_unsigned_as_double(CanFrame msg, uint64_t start,
                                        uint64_t length, double scale,
                                        double offset) {
    uint64_t bitfield = get_bitfield(can_word(msg), start, length);
...
/* Only works up to 32 bit */
float decode_signal_unsigned_as_float(CanFrame msg, uint64_t start,
                                      uint64_t length, double scale,
                                      double offset) {
    uint64_t bitfield = get_bitfield(can_word(msg), start, length);
    return apply_linear_double(bitfield, scale, offset);
...
int64_t decode_signal_signed_as_int64_t(CanFrame msg, uint64_t start,
                                        uint64_t length, double scale,
                                        double offset) {
...
int32_t decode_signal_signed_as_int32_t(CanFrame msg, uint64_t start,
                                        uint64_t length, double scale,
                                        double offset) {
...
int16_t decode_signal_signed_as_int16_t(CanFrame msg, uint64_t start,
                                        uint64_t length, double scale,
                                        double offset) {
...
int8_t decode_signal_signed_as_int8_t(CanFrame msg, uint64_t start,
                                      uint64_t length, double scale,
                                      double offset) {
...
double decode_signal_signed_as_double(CanFrame msg, uint64_t start,
                                      uint64_t length, double scale,
                                      double offset) {
...
float decode_signal_signed_as_float(CanFrame msg, uint64_t start,
                                    uint64_t length, double scale,
                                    double offset) {
...
uint64_t can_encode_signal_signed_from_float(float signal, uint64_t start,
                                             uint64_t length, double scale,
                                             double offset) {
    return set_bitfield(apply_linear_float(signal, 1 / scale, -offset), start,
                        length);
}

float decode_signal_float_as_float(CanFrame msg, uint64_t start,
                                   uint64_t length, double scale,
                                   double offset) {
    if (length != 32) {
        return 0.0;
...
double decode_signal_double_as_double(CanFrame msg, uint64_t start,
                                      uint64_t length, double scale,
                                      double offset) {
    if (length != 64) {
        return 0.0;
