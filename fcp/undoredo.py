...
class SQLiteUndoRedo:
    ...
    def _create_triggers(db, *args):
        ...
        db.execute("CREATE TABLE undolog(seq integer primary key, sql text, tbl text, key text)")
        for tbl in args:
            collist = db.execute(f"pragma table_info({tbl})").fetchall()

            sql = f"CREATE TRIGGER _{tbl}_it AFTER INSERT ON {tbl} BEGIN\n"
            sql += "  INSERT INTO undolog VALUES(NULL,"
            sql += f"'DELETE FROM {tbl} WHERE rowid='||new.rowid, '{tbl}', new.name);\nEND;\n"

            db.execute(sql)
            sql = f"CREATE TRIGGER _{tbl}_ut AFTER UPDATE ON {tbl} BEGIN\n"
            sql += "  INSERT INTO undolog VALUES(NULL,"
            sql += f"'UPDATE {tbl} "
            sep = "SET "
            for (x1, name, x2, x3, x4, x5) in collist:
                ...
                sep = ","
            sql += f" WHERE rowid='||old.rowid, '{tbl}', new.name);\nEND;\n"

            db.execute(sql)

            sql = f"CREATE TRIGGER _{tbl}_dt BEFORE DELETE ON {tbl} BEGIN\n"
            sql += "  INSERT INTO undolog VALUES(NULL,"
    ...
    def _step(self, v1, v2):
        ...
        q2 = f"SELECT tbl, key FROM undolog WHERE seq>={begin} AND seq<={end}" \
             " ORDER BY seq DESC"
        sqllist = self._db.execute(q1).fetchall()
        ids = self._db.execute(q2).fetchall()
