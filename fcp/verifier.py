...
class Verifier:
    ...
    def check_signal_type(signal):
        types = [signess + str(width) for signess in ['i', 'u'] for width in range(1,65)]
        types += ["f32", "f64"]
    ...
    def check_struct_duplicate_signals(struct):
        duplicates = list(Verifier.get_duplicates(struct.signals, lambda x: x.name, lambda x: x.name))
        if len(duplicates) == 0:
            return Ok(())
        else:
            return Error(f"Found duplicate singals in struct {struct.name}: {duplicates}")

    @staticmethod
    def check_enum_duplicated_values(enum):
        duplicates = list(Verifier.get_duplicates(enum.enumeration.items(), lambda x: x[1], lambda x: x[0]))
        if len(duplicates) == 0:
            return Ok(())
        else:
